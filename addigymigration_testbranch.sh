...
if [[ -e "/usr/local/bin/dialog" ]]; then
    ...
    if [[ "$swiftDialogVersion" == "2.4.2" ]]; then
        ...
        /usr/sbin/installer -pkg "/Library/Addigy/ansible/packages/Addigy Migration TESTING (2.0)/dialog-2.4.2-4755.pkg" -target /
    fi
else
...
if [[ "$currentDeferralCount" -gt 0 ]]; then
  ...
  while [ -z "$dialogResults" ]; do
    ...
    --icon none \
    --ontop \
    --image "/Library/Addigy/ansible/packages/Addigy Migration TESTING (2.0)/CoreLogoTransparent.png" \
    --button1text "Ready!" \
...
MDMLink="$MDMLink"
csvPath="/Library/Addigy/ansible/packages/Addigy Migration TESTING (2.0)/abm_devices.csv" # Export device list from ABM for devices expected to migrate
logoPath="/Library/Addigy/ansible/packages/Addigy Migration TESTING (2.0)/CoreLogoTransparent.png" # Core logo
...
sdTitleFontSize="24"
sdTitle="none"
sdIcon="/Library/Addigy/ansible/packages/Addigy Migration TESTING (2.0)/CoreLogoTransparent.png"
...
packageFolder="/Library/Addigy/ansible/packages/Addigy Migration TESTING (2.0)"
